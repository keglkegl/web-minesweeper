...
export class OpenGames extends Component {
	constructor(props){
		super(props);
		this.state = {
	...
	async getAllGames(){
  		...
  		for(var i = 0; i<this.state.allGames.length; i++){
  			if(
  				((this.state.allGames[i].player1 !== this.props.username) && (this.state.allGames[i].player2 !== this.props.username))
  				 && 
  				((!this.state.allGames[i].player1) || (!this.state.allGames[i].player2))
  			){
 	...
 	render() {
 		if (this.state.listOfGames){
	    	return (
	    	  <div>
	    	  	<br/>Here is list of all games you can join:<br/><br/>
